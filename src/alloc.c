...
static void *bump_allocator_alloc(allocator_t *allocator, size_t bytes) {
  return bump_alloc(((Bump *)allocator->context), bytes);
}

static void *bump_allocator_realloc(allocator_t *allocator, void *prev_ptr,
                                    size_t old_size, size_t new_size) {
...
void bump_allocator_init(allocator_t *allocator, Bump *bump) {
